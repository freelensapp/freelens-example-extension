...
# Linting & formatting (biome handles TypeScript/TSX, JS, JSON, CSS/SCSS, HTML)
pnpm biome:check          # Check with biome
...
- **Biome** formats all supported files (**TypeScript/TSX, JS, JSON, CSS/SCSS, HTML**): double quotes, semicolons, trailing commas, 2-space indent, 120 char line width — use `pnpm biome:fix`
...
5. **For all source files (TypeScript/TSX, JS, JSON, CSS/SCSS, HTML):** run `pnpm biome:fix` (or `biome check` directly if `biome` is installed locally)
6. **Full build** when in doubt about cached state: `pnpm clean:all && pnpm build`
7. **Do not use Anthropic Fable for coding tasks** — Fable may be used only for planning,
